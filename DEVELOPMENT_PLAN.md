...
>
> 开发日期：2026/07/06 — 2026/07/14
>
> **当前进度：P0/P1/P2 全部完成。检索质量评测完成（122 测试 + 30 条评测用例，Hit Rate 93.3%，MRR 0.933）。前端体验全面优化（对话删除/重命名/侧边栏收缩/文档分页/用户管理）。**
...
| GET | /api/documents?page=1&size=10 | 文档列表（分页） |
...
| PUT | /api/chat/sessions/{sessionId} | 重命名会话 |
| GET | /api/admin/users?page=1&size=10 | 用户列表（管理员） |
...
| 7/14 | **前端体验全面优化**：对话删除（前端删除按钮+后端 DELETE 接口永久删除）、对话标题重命名（双击内联编辑+后端 PUT 接口）、侧边栏收缩（桌面端「收缩为窄栏仅图标）、顶部栏动态标题（当前会话标题替代静态文字）、页面标签动态标题（`会话标题 - 校园智答` 模仿 DeepSeek）、标签页 SVG 图标、侧边栏 Logo、修复外侧滚动条、底部输入栏上移防任务栏遮挡、**文档管理分页**（后端 MyBatis-Plus Page + 前端 t-pagination，5/10条可选）、上传文档弹窗化（t-dialog 替换常驻卡片）、文档列表底部留白防分页被遮挡、**用户管理页面**（管理员专用，用户 CRUD + 分页 + 密码重置 + 启用/禁用） |
...
| 16 | 对话标题重命名 | 双击内联编辑 + 后端 PUT 接口 | ✅ |
| 17 | 文件 URL 安全 | id→随机 token，防遍历 | ⬜ |
| 18 | PROCESSING 补偿 | 重启扫描重新投 MQ | ⬜ |
| 19 | 对话删除功能 | 前端删除按钮 + 后端 DELETE 接口 | ✅ |
| 20 | 侧边栏收缩 | 桌面端「收缩为窄栏仅图标 | ✅ |
| 21 | 文档管理分页 | MyBatis-Plus Page + 前端 t-pagination，5/10条可选 | ✅ |
| 22 | 用户管理页面 | 管理员专用，用户 CRUD + 分页 + 密码重置 + 启用/禁用 | ✅ |
| 23 | 上传文档弹窗化 | t-dialog 替换常驻卡片 | ✅ |
...
### 12.7 真实 Embedding 全量验证（30 条）

启动完整系统（16 份真实文档 + DashScope Embedding + Milvus），逐条验证全部 30 条用例。
详细记录见 `src/test/resources/eval/real-embedding-results.md`。
...
关键词评测 4 条未命中（#8/#10/#22/#23）全部被真实 Embedding 修正。
...
> **最后更新：2026/07/14**
>
> **当前状态：** P0/P1/P2 全部完成。**122 个单元测试 + 30 条检索评测用例，关键词基线 Hit Rate 93.3%、MRR 0.933，真实 Embedding 预估 Hit Rate 97%+。** 前端体验全面优化（对话删除/重命名/侧边栏收缩/动态标题/文档分页/上传弹窗/用户管理）。剩余为体验优化项（文件URL安全/PROCESSING补偿）。
