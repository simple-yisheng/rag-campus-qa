...
#### 真实 Embedding 验证（DashScope text-embedding-v3）

关键词匹配未命中的 4 条用例，用真实 Embedding 语义检索重新验证：

| 用例 | 关键词基线 | 真实 Embedding | 相关度 |
|:---|:---|:---|:---|
| #8 材料 vs 软件学院综测对比 | ❌ 误匹配到无关文档 | ✅ 材料学院 96% + 软件学院 90% | 两份全部命中 |
| #10 一票否决行为有哪些 | ❌ 误匹配到软件学院 | ✅ 材料学院 88% + 软件学院 86% | 两份全部命中 |
| #22 三星奖学金面向学院+兼得规则 | ❌ 只找到部分 | ✅ 三星通知命中 | 正确 |
| #23 专项奖学金列表 | ❌ 误匹配到通知 | ✅ 摘要 93% + 三星通知 88% | 正确 |

**结论**：关键词检索的 93.3% Hit Rate 是保守下限。真实 Embedding 修正了全部关键词误判，综合分析：
