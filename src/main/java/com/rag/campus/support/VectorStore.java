 ...
 * 向量存储器 — 内存版
 * <p>
 * 内存中维护所有chunk向量，检索时计算余弦相似度返回Top-K。
 * <p>
 * 面试要点：
 * 1. Demo阶段用内存方案，生产可替换为 pgvector / Milvus / Redis Stack Vector Search
 * 2. 余弦相似度公式: cos(θ) = (A·B) / (|A| × |B|)
 * 3. 为什么设计为可替换？— 策略模式，VectorStore接口化后可注入不同实现
 * <p>
 * 性能说明：
 * - 1000个1024维向量，遍历计算余弦相似度约耗时 5-10ms
 * - 万级以内都能接受，超过10万建议上专业向量数据库
 */
@Slf4j
@Component
...
public class VectorStore {
    ...
    public List<Hit> search(float[] queryVector, int topK) {
        ...
        // 计算所有chunk与查询向量的余弦相似度
        PriorityQueue<Hit> minHeap = new PriorityQueue<>(
                Comparator.comparingDouble(Hit::getScore)
        );
        ...
        for (Map.Entry<Long, float[]> entry : vectorIndex.entrySet()) {
            ...
            Hit hit = new Hit(entry.getKey(), (float) similarity);
        ...
        // 转为降序列表
        List<Hit> results = new ArrayList<>(minHeap);
