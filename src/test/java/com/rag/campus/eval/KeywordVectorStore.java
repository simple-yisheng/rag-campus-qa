...
/**
 * 基于关键词匹配的模拟向量存储（仅用于检索管道评测）。
 * <p>
 * <b>重要：这不是真实的向量召回。</b>
 * 它用字符级 n-gram 关键词重叠度模拟语义相似度打分，
 * 目的是消除 Embedding API 的不确定性，聚焦测试检索<b>后处理管道</b>：
 * 关键词加权 → 标题匹配 → 双阈值过滤 → 多样性截断。
 * <p>
 * <b>真实语义召回效果会显著优于这里的模拟结果。</b>
 * 本评测得到的是检索管道的<b>保守下限</b>（lower bound）。
 * <p>
 * 匹配策略：
 * 1. 查询文本切分为 2/3/4-gram + 空格分词双路
 * 2. chunk 得分 = 命中词数 / 查询总词数（归一化到 0-1）
